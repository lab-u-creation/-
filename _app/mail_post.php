...
$site_top = appConfig::baseUrl;

//管理者のメールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
if (ENV === "release") {
	$to = appConfig::mailAddres; //本番環境で使用するメールアドレス
} else {
	$to = appConfig::mailAddres_test; //テスト環境で使用するメールアドレス
}

//自動返信メールの送信元メールアドレス
//必ず実在するメールアドレスでかつ出来る限り設置先サイトのドメインと同じドメインのメールアドレスとすることを強く推奨します
if (ENV === "release") {
	$from  = appConfig::mailAddres; //本番環境で使用するメールアドレス
} else {
	$from  = appConfig::mailAddres_test; //テスト環境で使用するメールアドレス
}
...
$Referer_check_domain = "php-factory.net";/*http://osoushiki-tokyo.sakura.ne.jp/*//*php-factory.net*/
...
//会社の電話番号
$companyTel = appConfig::companyTel;
...
/* mail_post.php標準のお問い合わせ（見本用・使わない）*/
$subject = "お問い合わせ"; // 管理者宛に送信されるメールのタイトル（件名）
$require = array('入力内容'); //必須入力項目
$blankremove = array(); // POSTされた値がブランクの時、除去される項目
$thanksPage = "./thanks"; // サンクスページ

/* 終活フェアー申込み*/
$item_shukatsu_postType = "shukatsu";
$item_shukatsu_subject = "終活フェアー申込み";
$item_shukatsu_require = array('ご希望のイベント', '場所', 'ご希望日時', '参加人数', 'お名前', 'フリガナ', '電話番号');
$item_shukatsu_blankremove = array();
...
if (isset($_POST['post_type'])) {
	switch ($_POST['post_type']) {
		case $item_shukatsu_postType:
			//終活フェアー申込みの場合
			$subject = $item_shukatsu_subject;
			$require = $item_shukatsu_require;
			$blankremove = $item_shukatsu_blankremove;
			$companyTel = '0120-007-950';
			break;
		default:
			//それ以外のメールの場合
			break;
...
//空白チェック実行し返り値を受け取る
$_POST = blankremove($blankremove, $_POST);
...
if (($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1) {
	...
	if ($empty_flag == 1) {
		$body = "";
		$body = <<<EOF
<p class="text-center pdb-none">入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します</p>
<div class="container">
<div class="pdt-large pdb-large">
{$errm}
</div>
<div class="pd-large">
<button class="btn border border-gray icon-arrow-left font-size-middle-l pd-small w-50per" onClick="history.back()">前画面に戻る</button>
</div>
</div>
EOF;
	} else {
		$body = "";
		$formAction = h($_SERVER['SCRIPT_NAME']);
		$formPost = confirmOutput($_POST);
		$httpReferer = h($_SERVER['HTTP_REFERER']);
		$body = <<<EOF
<p class="text-center pdb-none">以下の内容で間違いがなければ、「送信する」ボタンを押してください。</p>
<div class="container">
<form action="{$formAction}" method="POST" class="pd-middle">
<div class="pdb-large">
...
<div class="row w-80per mx-auto">
<div class="col-12 col-md-6 pdb-middle"><button class="btn border border-red border-bold icon-arrow-left font-size-middle-l pd-small w-100per">送信する</button></div>
<div class="col-12 col-md-6 pdb-middle"><button type="button" class="btn border border-gray icon-arrow-left font-size-middle-l pd-small w-100per" onClick="history.back()">前画面に戻る</button></div>
</div>
</form>
EOF;
	}
...
function confirmOutput($arr)
{
	...
	foreach ($arr as $key => $val) {
		...
		if ($key != 'post_type') {
			$html .= '<tr><th class="bg-lgray w-30per">' . $key . '</th><td>' . $out;
...
function mailToAdmin($arr, $subject, $mailFooterDsp, $mailSignature, $encode, $confirmDsp)
{
	...
	$adminBody .= "\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n";
	$adminBody .= "メールアドレス記入のお客様には、自動返信メールを送信済みです。" . "\n";
	$adminBody .= "(参考)http://osoushiki-tokyo.jp/mailsample.html" . "\n\n";
...
//値が空白の要素は除去する
function blankremove($blankremove, $post)
{
	foreach ($blankremove as $blankremoveVal) {
		foreach ($_POST as $key => $val) {
			if ($key == $blankremoveVal) {
				//配列のためのチェック
				if (is_array($val)) {
					$connectEmpty = 0;
					foreach ($val as $val2) {
