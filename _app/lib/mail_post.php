...
$site_top = appConfigSite::baseurl;

//サイトの名称
$site_name = appConfigSite::siteName;

//管理者のメールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
if (ENV === "release") {
	$to = appConfigSite::mailAddres; //本番環境で使用するメールアドレス
} else {
	$to = appConfigSite::mailAddres_test; //テスト環境で使用するメールアドレス
}

//自動返信メールの送信元メールアドレス
//必ず実在するメールアドレスでかつ出来る限り設置先サイトのドメインと同じドメインのメールアドレスとすることを強く推奨します
if (ENV === "release") {
	$from  = appConfigSite::mailAddres; //本番環境で使用するメールアドレス
} else {
	$from  = appConfigSite::mailAddres_test; //テスト環境で使用するメールアドレス
}
...
//会社の住所
$companyAddress = appConfigSite::address;

//会社の電話番号
$companyTel = appConfigSite::tel;

//ウェブサイトのURL
$baseUrl = appConfigSite::baseurl;
...
$refrom_name = appConfigSite::siteName;
...
if (($jumpPage == 0 && $sendmail == 1) || ($jumpPage == 0 && ($confirmDsp == 0 && $sendmail == 0))) {
	...
	echo '<p>エラーが発生しました（エラー番号：01）</p>';
	/* ▲▲▲送信完了画面のレイアウト 編集可 ※送信完了後に指定のページに移動しない場合のみ表示▲▲▲　*/
} else if (($jumpPage == 1 && $sendmail == 1) || $confirmDsp == 0) {
	//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	if ($empty_flag == 1) {
		echo '<p>エラーが発生しました（エラー番号：01）</p>';
		echo '<p>入力にエラーがあります。下記をご確認の上「戻る」ボタンにて修正をお願い致します。</p>';
		echo $errm;
		echo '<input type="button" value="前画面に戻る " onClick="history.back()">';
	} else {
